...
$availablePlans = SubscriptionManager::allPlans();
$subscriptionPlanOptions = [
    'RaceVerse BASIC' => 'RaceVerse BASIC',
    'RaceVerse PRO' => 'RaceVerse PRO (manuale)',
];
foreach ($availablePlans as $plan) {
    $subscriptionPlanOptions[$plan['name']] = $plan['name'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_hotlap') {
        ...
        $expiresInput = $_POST['subscription_expires_at'] ?? '';
        ...
        if (!$userId) {
            ...
            $expiresDate = admin_from_datetime_input($expiresInput);

            $planDefinition = null;
            foreach ($availablePlans as $definition) {
            ...
            $isProPlan = strpos((string)$plan, 'RaceVerse PRO') !== false;
            if (!$isProPlan) {
            ...
            if (!$isProPlan || !$active) {
                ...
                if (!$expiresDate) {
                    $days = $planDefinition['duration_days'] ?? 30;
                    $expiresDate = date('Y-m-d H:i:00', strtotime('+' . (int)$days . ' days'));
                }
                if ($paymentMethod === '') {
            ...
            try {
                $sql = 'UPDATE users SET role = :role, subscription_plan = :plan, subscription_active = :active, subscription_started_at = :started_at, subscription_expires_at = :expires_at, subscription_payment_method = :payment_method';
                $params = [
                    ':role' => $role,
                    ':plan' => $plan,
                    ':active' => $active,
...
if ($pdo) {
    ...
    $users = $pdo->query('SELECT id, email, first_name, last_name, role, subscription_plan, subscription_active, subscription_started_at, subscription_expires_at, subscription_payment_method, created_at FROM users ORDER BY created_at DESC')->fetchAll();
}
                ...
                <form method="post" class="space-y-3">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    ...
                    <label class="space-y-1">
                      ...
                      <?php $currentPlan = $user['subscription_plan'] ?: 'RaceVerse BASIC'; ?>
                      <select name="subscription_plan" class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10">
                        <?php foreach ($subscriptionPlanOptions as $value => $label): ?>
                          <option value="<?= htmlspecialchars($value) ?>" <?= $currentPlan === $value ? 'selected' : '' ?>><?= htmlspecialchars($label) ?></option>
                        <?php endforeach; ?>
                      </select>
                    </label>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    ...
                    <label class="space-y-1">
                      ...
                      <input type="datetime-local" name="subscription_expires_at" value="<?= htmlspecialchars(admin_to_datetime_value($user['subscription_expires_at'])) ?>" class="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/10">
                    </label>
                  </div>
                  <label class="block text-sm space-y-1">
