...
$pdo = Database::pdo();
$sql = "SELECT cs.file_slug, cs.notes, cs.created_at, cs.file_path,
               c.name AS car_name,
...
$storageDir = realpath(__DIR__ . '/assets/files');
$storedFile = $setup['file_path'] ?? null;

if ($storageDir && $storedFile) {
    $filePath = $storageDir . DIRECTORY_SEPARATOR . $storedFile;
    if (is_file($filePath)) {
        $safePath = realpath($filePath);
        if ($safePath && strpos($safePath, $storageDir) === 0) {
            $extension = pathinfo($safePath, PATHINFO_EXTENSION);
            $downloadName = slugify($setup['track_name']) . '-' . slugify($setup['car_name']);
            if ($extension) {
                $downloadName .= '.' . $extension;
            }
            $mime = function_exists('mime_content_type') ? mime_content_type($safePath) : 'application/octet-stream';
