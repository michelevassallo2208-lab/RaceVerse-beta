  ...
  <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-white/60">Ruolo: <strong><?= htmlspecialchars($user['role']) ?></strong> • Piano: <strong><?= htmlspecialchars($user['subscription_plan'] ?: 'RaceVerse BASIC') ?></strong> <?= $user['subscription_active'] ? '✅' : '⌛' ?></p>
      </div>
    </div>
  ...
  <div class="grid md:grid-cols-3 gap-6">
    <div class="md:col-span-2 space-y-4">
      <div class="p-6 rounded-2xl bg-white/5 border border-white/10">
        ...
        <p class="text-sm text-white/70 mb-4">Sblocca report avanzati, setup esclusivi e supporto prioritario con l'abbonamento mensile da €2,99.</p>
        <?php if (!Auth::isPro()): ?>
          <a href="<?= asset('payment.php') ?>" class="inline-flex px-5 py-2 rounded-xl bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 text-black font-semibold shadow-lg shadow-emerald-500/30">Vai alla pagina abbonamento</a>
        <?php else: ?>
          <span class="inline-flex px-4 py-2 rounded-xl bg-emerald-600/25 border border-emerald-400/40 text-emerald-200 text-sm">Abbonamento PRO attivo</span>
        <?php endif; ?>
      </div>
      <div class="p-6 rounded-2xl bg-white/5 border border-white/10">
