...
<section class="rounded-3xl p-8 md:p-14 bg-gradient-to-br from-indigo-600/30 via-fuchsia-500/20 to-emerald-400/20 border border-white/10 shadow-2xl shadow-indigo-500/20 overflow-hidden relative">
  ...
  <div class="relative max-w-3xl space-y-4">
    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-black/30 border border-white/20 text-sm uppercase tracking-[0.3em]">Premium Racing Data</span>
    <h1 class="text-4xl md:text-5xl font-black leading-tight">Trova l'auto perfetta, domina ogni hotlap.</h1>
    <p class="text-white/80 text-lg">MetaSim+ raccoglie i migliori tempi dei pro, li analizza e ti offre i setup più performanti. Dati real-time, look premium.</p>
    <div class="flex flex-col sm:flex-row gap-3">
      <a href="#selector" class="px-6 py-3 rounded-2xl bg-white text-black font-semibold shadow-lg shadow-white/40 hover:shadow-white/60 transition">Esplora leaderboard</a>
      <a href="#pro" class="px-6 py-3 rounded-2xl bg-black/40 border border-white/20 text-white/90 hover:text-white">Scopri MetaVerse Pro</a>
    </div>
  </div>
</section>
...
<section id="insights" class="grid md:grid-cols-3 gap-5">
  <div class="md:col-span-2 p-6 rounded-3xl bg-black/30 border border-white/10 shadow-lg shadow-emerald-500/10">
    ...
    <div class="grid md:grid-cols-2 gap-4 text-sm text-white/70">
      ...
      <div class="p-4 rounded-2xl bg-white/5 border border-white/10">
        ...
        <div>Vantaggio medio ottenuto dagli utenti MetaVerse Pro.</div>
      </div>
    </div>
  </div>
  <div class="p-6 rounded-3xl bg-gradient-to-br from-indigo-500/30 via-purple-500/20 to-emerald-500/30 border border-white/10 shadow-xl shadow-purple-500/20 space-y-3">
    <h3 class="text-xl font-semibold">Perché MetaSim+</h3>
    <ul class="space-y-3 text-sm text-white/80">
      ...
      <li class="flex gap-3"><span class="mt-1 w-2 h-2 rounded-full bg-indigo-400"></span> Visual design premium, pensato per l'esperienza desktop e mobile.</li>
      <li class="flex gap-3"><span class="mt-1 w-2 h-2 rounded-full bg-fuchsia-400"></span> Filtri smart e risposta immediata per trovare il giro perfetto.</li>
    </ul>
  </div>
</section>
...
<section id="pro" class="rounded-3xl p-8 md:p-10 bg-black/40 border border-emerald-400/40 shadow-xl shadow-emerald-500/20">
  <div class="grid md:grid-cols-2 gap-8 items-center">
    <div class="space-y-4">
      <h2 class="text-3xl font-black text-emerald-200">MetaVerse Pro</h2>
      <p class="text-white/70 text-lg">Accedi alla libreria dei setup ufficiali, strategie di stint e report telemetrici esportabili. Abbonamento su misura per team e piloti che puntano al podio.</p>
      <ul class="space-y-2 text-white/80 text-sm">
        <li>• Setup premium scaricabili con un clic</li>
        <li>• Analisi comparativa con delta e ghost</li>
        <li>• Supporto dedicato e richieste personalizzate</li>
      </ul>
      <div class="flex gap-3">
        <a href="<?= asset('login.php') ?>" class="px-6 py-3 rounded-2xl bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 text-black font-semibold shadow-lg shadow-emerald-500/40">Accedi all'area Pro</a>
        <a href="#" class="px-6 py-3 rounded-2xl bg-white/10 border border-white/20 text-white/80 hover:text-white">Richiedi demo</a>
      </div>
    </div>
    <div class="relative">
      ...
      <div class="relative rounded-3xl border border-white/10 bg-white/5 p-6 space-y-4 shadow-2xl shadow-emerald-500/20">
        <div class="text-sm uppercase tracking-[0.4em] text-white/60">Performance pack</div>
        <div class="text-4xl font-black">€19<span class="text-lg font-semibold text-white/70">/mese</span></div>
        <p class="text-white/70 text-sm">Cancella quando vuoi. Accesso immediato a setup, hotlap onboard e feedback degli ingegneri.</p>
        <button class="w-full py-3 rounded-2xl bg-white text-black font-semibold">Attiva ora</button>
      </div>
    </div>
  </div>
</section>
...
async function search(){
  ...
  const url = buildUrl(`api/hotlaps.php?game=${encodeURIComponent(gameEl.value)}&category=${encodeURIComponent(catEl.value)}&track=${encodeURIComponent(trackEl.value)}`);
...
gameEl.addEventListener('change', async ()=>{
  ...
  const res = await fetch(buildUrl('api/tracks.php?game='+encodeURIComponent(gameEl.value)));
