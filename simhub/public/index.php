...
<section id="selector" class="rounded-3xl p-6 md:p-8 bg-white/5 border border-white/10 shadow-xl shadow-indigo-500/10">
  ...
  <div id="setup-action" class="hidden mt-4"></div>
</section>
...
async function search(){
  ...
  try {
    ...
    if (!data.length){
      ...
      if (setupAction){ setupAction.classList.add('hidden'); setupAction.innerHTML=''; }
      return;
    }
    results.innerHTML = data.map((x,i)=>card(x,i)).join('');
    if (setupAction) {
      if (setupAccess.canDownload) {
        setupAction.innerHTML = `<a href="${setupAccess.downloadUrl}" class="inline-flex items-center justify-center px-5 py-3 rounded-xl bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 text-black font-semibold shadow-lg shadow-emerald-500/30">Download Assetto</a>`;
      } else {
        setupAction.innerHTML = `<a href="${setupAccess.ctaUrl}" class="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white/10 border border-white/20 text-white/80 hover:text-white transition">${setupAccess.ctaLabel}</a>`;
      }
