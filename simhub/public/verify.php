...
if ($token) {
  try {
    $pdo = Database::pdo();
    $st = $pdo->prepare('SELECT id, email, email_verified_at, subscription_active FROM users WHERE verification_token = ? LIMIT 1');
    $st->execute([$token]);
    $user = $st->fetch();
    if ($user) {
      if (!empty($user['email_verified_at']) && (int)$user['subscription_active'] === 1) {
        $status = 'info';
        $message = 'Questa email è già stata verificata. Puoi accedere a RaceVerse PRO.';
      } else {
        $update = $pdo->prepare('UPDATE users SET email_verified_at = NOW(), subscription_active = 1, verification_token = NULL WHERE id = ?');
        $update->execute([$user['id']]);
        $status = 'success';
        $message = 'Email verificata con successo! Ora puoi accedere a RaceVerse PRO.';
      }
    } else {
...
<section class="max-w-3xl mx-auto rounded-3xl p-10 bg-black/40 border border-white/10 shadow-2xl shadow-emerald-500/20 text-center space-y-6">
  ...
  <h1 class="text-3xl font-bold">Verifica RaceVerse PRO</h1>
