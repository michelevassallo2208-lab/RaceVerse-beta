  ...
  if (!$errors) {
    try {
      ...
      if ($existing->fetch()) {
        ...
        $stmt = $pdo->prepare('INSERT INTO users (email, password_hash, first_name, last_name, verification_token, subscription_plan, subscription_active, subscription_started_at, subscription_expires_at, subscription_payment_method) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?, ?)');
        $stmt->execute([
          $email,
    ...
    <div class="p-6 rounded-2xl bg-white/5 border border-white/10">
      ...
      <?php else: ?>
        ...
        <form method="post" class="space-y-4">
          ...
          <p class="text-xs text-white/50 text-center">Registrandoti accetti i termini di utilizzo di RaceVerse. L'upgrade a RaceVerse PRO parte da €2,99 per 30 giorni e potrà essere attivato dopo la conferma dell'account.</p>
        </form>
      <?php endif; ?>
    </div>
  </div>
