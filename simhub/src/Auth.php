...
class Auth {
  ...
  public static function login(string $email, string $password): bool {
    self::start();
    $pdo = Database::pdo();
    $st = $pdo->prepare("SELECT id,email,password_hash,role,subscription_plan,subscription_active,subscription_started_at,subscription_expires_at,subscription_payment_method FROM users WHERE email=? LIMIT 1");
    $st->execute([$email]);
    $u = $st->fetch();
